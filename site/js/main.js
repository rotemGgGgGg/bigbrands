...
(function () {
  ...
  /* ---------- placeholder device graphic (no image files needed) ---------- */
  function deviceSVG(accent) {
    accent = accent || "#1f7ae0";
    const gid = "scr-" + Math.random().toString(36).slice(2, 8);
    return `
    <svg class="device" viewBox="0 0 220 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="מוצר">
      <defs><linearGradient id="${gid}" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stop-color="${accent}"/><stop offset="1" stop-color="#0a1f44"/>
      </linearGradient></defs>
      ...
      <rect x="92" y="112" width="36" height="6" rx="3" fill="#9aa6bd"/>
    </svg>`;
  }
  ...
  function productMedia(p) {
    if (p.image) {
      return `<img class="device" src="${escapeHtml(p.image)}" alt="${escapeHtml(p.name)}" loading="lazy"
        data-accent="${brandColor(p.brand)}" style="object-fit:contain" onerror="window.__productFallback(this)"/>`;
    }
